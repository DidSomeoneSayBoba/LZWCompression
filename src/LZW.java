...
public class LZW {
	...
	public void convertToBinary(File file) throws IOException {
		...
		while (reader.ready()) {
			...
			if (!dict.contains(current + next) && dict.size() < Math.pow(2, NUM)) {
				...
				if (dict.size() >= Math.pow(2, NUM)) {
					System.out.println("Alert: dict has too many values");
				}
				current = next;
	...
	public void decompress(String compressed, String outname) throws IOException {// changed to void to match
																					// compression
		long start = System.currentTimeMillis();
		ArrayList<String> dict = new ArrayList(initialdict);
